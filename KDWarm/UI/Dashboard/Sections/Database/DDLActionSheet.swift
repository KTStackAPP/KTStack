...
struct DDLActionSheet: View {
    ...
    private func removeColumn(_ id: UUID) { rows.removeAll { $0.id == id } }

    private func definitions() -> [ColumnDefinition] {
        rows.map {
            ColumnDefinition(name: $0.name.trimmingCharacters(in: .whitespaces),
                             type: $0.type, isNullable: $0.nullable, isPrimaryKey: $0.primaryKey)
        }
    }
    ...
    private func compose() {
        switch mode {
        case .createTable:
            vm.prepareCreateTable(name: tableName.trimmingCharacters(in: .whitespaces),
                                  columns: definitions())
        case .addColumn:
