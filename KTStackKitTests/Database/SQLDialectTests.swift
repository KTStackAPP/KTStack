...
final class SQLDialectTests: XCTestCase {
    ...
    // MARK: - quoteIdent (MySQL: backtick)
    ...
    // MARK: - quoteIdent (PostgreSQL / SQLite: double-quote)
    ...
    // MARK: - quoteIdent rejects control characters
    ...
    func testPaginateAppendsLimitOffset() {
        let d = SQLDialect.forKind(.mysql)
        XCTAssertEqual(d.paginate("SELECT * FROM `t`", limit: 50, offset: 0),
                       "SELECT * FROM `t` LIMIT 50 OFFSET 0")
        XCTAssertEqual(d.paginate("SELECT * FROM `t`", limit: 25, offset: 100),
    ...
    func testPaginateClampsNonPositiveLimitAndNegativeOffset() {
        ...
        XCTAssertEqual(d.paginate("SELECT 1", limit: 0, offset: -5),
