#!/usr/bin/env bash
# Notarize a signed KTStack.app: zip it, submit to Apple, wait, then staple the ticket. Apple
...
#   xcrun notarytool store-credentials ktstack-notary --apple-id you@example.com --team-id TEAMID --password APP-SPECIFIC-PW
# Usage: scripts/release/notarize.sh KTStack.app [keychain-profile]
set -euo pipefail
APP="${1:?usage: notarize.sh <path-to-.app> [keychain-profile]}"
PROFILE="${2:-ktstack-notary}"
