...
final class MongoDriverIntegrationTests: XCTestCase {
    private func makeDriver() throws -> MongoDriver {
        ...
        return MongoDriver(profile: .managedMongo, password: nil, tools: FakeDatabaseTools.allInstalled)
    }
    ...
    func testReadOnlyConnectionRefusesWrites() async throws {
        ...
        let driver = MongoDriver(profile: readOnly, password: nil, tools: FakeDatabaseTools.allInstalled)
        do {
