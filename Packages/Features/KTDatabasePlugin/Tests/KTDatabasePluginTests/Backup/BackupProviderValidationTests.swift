...
final class BackupProviderValidationTests: XCTestCase {
    ...
    func testFactorySurfacesReasonWhenProviderUnavailable() throws {
        ...
        if case let .unavailable(reason) = BackupProviderFactory.make(for: .mysql, tools: FakeDatabaseTools()) {
            XCTAssertTrue(reason.lowercased().contains("mysql"))
        }
        if case let .unavailable(reason) = BackupProviderFactory.make(for: .mongodb, tools: FakeDatabaseTools()) {
    ...
    func testSessionRefusesIncompatibleMajorVersion() async throws {
        ...
        let session = BackupSession(library: library, tools: FakeDatabaseTools()) { kind in
    ...
    func testManagedSessionResolvesEngineVersionFromTools() throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("ktstack-managed-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tmp, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: tmp) }
        let tools = FakeDatabaseTools(versions: [.postgres: "17.10"])
        let session = BackupSession.managed(tools: tools, paths: AppSupportPaths(root: tmp))
