...
final class DatabaseViewModelSessionTests: XCTestCase {
    ...
    func testOpensSessionOnceAfterConnecting() async {
        ...
        let vm = DatabaseViewModel(tools: FakeDatabaseTools(), makeDriver: { _, _ in driver }, passwordFor: { _ in nil })
    ...
    func testFailedConnectionDoesNotOpenSession() async {
        let vm = DatabaseViewModel(tools: FakeDatabaseTools(), makeDriver: { _, _ in nil }, passwordFor: { _ in nil })
