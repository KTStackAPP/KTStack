...
final class DocumentViewModelTests: XCTestCase {
    ...
    private func makeVM(_ driver: StubDocumentDriver) -> DocumentViewModel {
        DocumentViewModel(tools: FakeDatabaseTools(), makeDriver: { _, _ in driver }, passwordFor: { _ in nil })
    }
    ...
    func testUnsupportedEngineFailsCleanly() async {
        let vm = DocumentViewModel(tools: FakeDatabaseTools(), makeDriver: { _, _ in nil }, passwordFor: { _ in nil })
