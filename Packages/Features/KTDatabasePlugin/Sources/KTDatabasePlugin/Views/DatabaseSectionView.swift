...
struct DatabaseSectionView: View {
    ...
    private var session: BackupSession { plugin.backupSession }
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            Spacer()
            KTButton(title: "Connect", systemImage: "link", kind: .secondary) { sectionState.connectPresented = true }
            ...
            KTButton(title: "New Database", systemImage: "plus", kind: .primary) { sectionState.newDatabasePresented = true }
    ...
    private var serversTab: some View {
        if visibleProfiles.isEmpty {
            emptyState(
                ...
                cta: ("Connect a server", "link", { sectionState.connectPresented = true })
            )
        } else {
            VStack(spacing: 14) {
        ...
        Task {
            ...
            if profile.kind == .mongodb {
                ...
                guard vm.connection == .connected else {
                    if case let .failed(error) = vm.connection { feedback.toast(error.message) }
                    return
                }
                if let database = vm.resolvePreferredDatabase(for: profile) {
                    await vm.select(database: database)
                } else {
                    feedback.toast("Connected to \"\(profile.name)\" - no databases found")
                }
                plugin.openSQLEditor(profile)
            }
        }
    }
