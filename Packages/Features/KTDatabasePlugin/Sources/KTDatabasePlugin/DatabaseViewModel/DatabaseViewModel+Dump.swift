...
public extension DatabaseViewModel {
    ...
    var canManualImport: Bool {
        ...
        case .postgres, .sqlite:
            if case .available = BackupProviderFactory.make(for: kind, tools: tools) { return true }
            return false
        case .mongodb:
            return false
        }
    ...
    var canCreateDatabase: Bool {
        ...
        case .mysql:
            return dumpService.isEngineInstalled
        case .postgres:
            if case .available = BackupProviderFactory.make(for: .postgres, tools: tools) { return true }
            return false
        case .sqlite, .mongodb:
            return false
        }
    ...
    var importUnavailableReason: String? {
        ...
        case .postgres, .sqlite:
            switch BackupProviderFactory.make(for: kind, tools: tools) {
    ...
    func targetDatabaseExists(_ name: String) async -> Bool {
        ...
        case .postgres:
            return await (try? PostgresBackupProvider(tools: tools).databaseExists(
    ...
    func importDatabase(into database: String, from input: URL, replaceExisting: Bool) async {
        ...
        do {
            ...
            case .postgres:
                try await PostgresBackupProvider(tools: tools).importManual(
