...
struct APIRequestPanel: View {
    ...
    @State var builderTab: BuilderTab = .params
    ...
    private var methodPicker: some View {
        let tint = APIMethodStyle.tint(vm.draftMethod)
        return Menu {
    ...
    private var responseArea: some View {
        if let error = vm.sendError {
            ...
            APIResponseView(response: response, bodyLimitKB: vm.bodyDisplayLimitMB * 1024)
                .frame(maxWidth: .infinity, maxHeight: .infinity)
        } else {
            VStack(spacing: 6) {
