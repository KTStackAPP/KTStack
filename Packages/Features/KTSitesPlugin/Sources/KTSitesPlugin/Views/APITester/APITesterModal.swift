...
struct APITesterModal: View {
    let site: SiteSummary
    let onClose: () -> Void

    @StateObject private var vm: APITesterViewModel

    init(site: SiteSummary, routes: any APIRouteIntrospecting, onClose: @escaping () -> Void) {
        self.site = site
        self.onClose = onClose
        _vm = StateObject(wrappedValue: APITesterViewModel(routeIntrospection: routes))
    }
    ...
    private var card: some View {
        ...
        .sheet(isPresented: $vm.isEditingVariables) {
            APIVariablesSheet(vm: vm, site: site)
        }
    }
    ...
    private func body(for loadError: String?) -> some View {
        if let loadError, vm.routes.isEmpty, !vm.isLoadingRoutes {
            ...
            HStack(spacing: 0) {
                APIRouteSidebar(vm: vm)
                APIRequestPanel(vm: vm, site: site)
            }
        }
    }
