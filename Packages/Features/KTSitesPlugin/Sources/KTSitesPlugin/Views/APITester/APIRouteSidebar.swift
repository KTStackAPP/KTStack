...
struct APIMethodBadge: View {
    let method: String

    var body: some View {
        let tint = APIMethodStyle.tint(method)
...
struct APIRouteSidebar: View {
    ...
    private func row(_ route: APIRoute) -> some View {
        ...
        return Button { vm.select(route) } label: {
            HStack(spacing: 9) {
                APIMethodBadge(method: route.method).frame(width: 52, alignment: .leading)
