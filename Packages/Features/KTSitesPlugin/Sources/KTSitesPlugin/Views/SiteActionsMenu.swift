...
struct SiteActionsMenu: View {
    ...
    private var header: some View {
        HStack(spacing: 10) {
            KTIconTile(tint: SiteVisuals.tint(for: site.kind), size: 32) {
                KTSiteGlyph(
    ...
    private func row(
        ...
        SiteActionRow(title: title, symbol: symbol, shortcut: shortcut, enabled: enabled, danger: danger) {
            open = false
            action()
        }
    }
