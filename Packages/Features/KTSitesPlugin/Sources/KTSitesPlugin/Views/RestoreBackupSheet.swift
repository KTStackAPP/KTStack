...
struct RestoreBackupSheet: View {
    ...
    let isEndOfLife: (String) -> Bool
    @Environment(\.dismiss) private var dismiss
    @StateObject private var model: RestoreBackupModel
    @State private var showPicker = false

    init(site: SiteSummary, restoring: any WordPressRestoring, availableVersions: [String], isEndOfLife: @escaping (String) -> Bool) {
        self.site = site
        self.availableVersions = availableVersions
        self.isEndOfLife = isEndOfLife
        _model = StateObject(wrappedValue: RestoreBackupModel(site: site, restoring: restoring))
    }
    ...
    var body: some View {
        ...
        .fileImporter(isPresented: $showPicker, allowedContentTypes: allowedTypes) { result in
            if case let .success(url) = result {
                ...
                model.selectFile(url, installed: availableVersions)
        ...
        HStack {
            ...
            Picker("", selection: $model.phpVersion) {
                ForEach(availableVersions, id: \.self) { version in
                    Text(isEndOfLife(version) ? "\(version) (EOL)" : version).tag(version)
                }
            }
            .labelsHidden()
            .frame(width: 140)
            if availableVersions.isEmpty {
                Text("No PHP installed — install a runtime first.")
                    .font(KDFont.footnote).foregroundStyle(Color.KDStatus.error)
            } else if isEndOfLife(model.phpVersion) {
    ...
    private var successView: some View {
        VStack(alignment: .leading, spacing: KDSpacing.space3) {
            ...
            HStack {
                Spacer()
                Button("Open Site") { SiteActions.openInBrowser(site) }
