...
struct DraftDataTabView: View {
    ...
    var body: some View {
        DraftChrome(activeTab: .data) {
            VStack(spacing: 0) {
                ...
                filterBar
                HStack(spacing: 0) {
                    DraftGrid(columnTitles: columnTitles,
                              rows: table.rows,
    ...
    private var columnTitles: [String] { table.columns.map(\.name) }
    ...
    private func detailValue(_ cell: DraftCell) -> some View {
        switch cell {
        case .text(let value): Text(value).font(.jbMono(12.5)).foregroundStyle(KTEditorTheme.label)
        case .number(let value): Text(value).font(.jbMono(12.5)).foregroundStyle(KTEditorTheme.label)
        case .foreign(let value): Text(value).font(.jbMono(12.5)).foregroundStyle(KTEditorTheme.accent)
