...
struct DraftDatabaseScreenView: View {
    private let servers: [DraftServer] = [
        DraftServer(name: "phongda", engine: "MySQL", engineTint: Color(hex: 0x1FA463),
                    engineBackground: Color(hex: 0xE7F8EE), badge: nil,
                    status: .online("Online · 127.0.0.1:3306 · 23 databases")),
        DraftServer(name: "MySQL (managed)", engine: "MySQL", engineTint: Color(hex: 0x1FA463),
                    engineBackground: Color(hex: 0xE7F8EE), badge: "bundled",
                    status: .online("Online · 127.0.0.1:3306 · 5 databases")),
        DraftServer(name: "PostgreSQL (managed)", engine: "Postgres", engineTint: Color(hex: 0x2F6BFF),
                    engineBackground: Color(hex: 0xEAF1FF), badge: "bundled",
                    status: .connecting("Connecting… · 127.0.0.1:5432")),
        DraftServer(name: "MongoDB (managed)", engine: "Mongo", engineTint: Color(hex: 0x13AA52),
                    engineBackground: Color(hex: 0xE5F7EC), badge: "bundled",
    ...
    private func serverRow(_ server: DraftServer) -> some View {
        let isOffline: Bool = { if case .offline = server.status { return true } else { return false } }()
    ...
    private func statusLabel(_ status: DraftServerStatus) -> some View {
        switch status {
        case .online(let text): statusRow(text, tint: KTEditorTheme.Status.running)
        case .connecting(let text): statusRow(text, tint: KTEditorTheme.accent)
        case .offline(let text): statusRow(text, tint: KTEditorTheme.label3)
        }
    }
