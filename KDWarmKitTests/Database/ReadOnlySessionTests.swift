...
/// on `KDWARM_DB_IT=1` + an installed, running managed MySQL on :3306, so a clean CI box skips rather
...
final class ReadOnlySessionTests: XCTestCase {
    private let scratchDB = "kdwarm_ro_probe"
    ...
    private func skipUnlessEngine() throws {
        ...
        try XCTSkipUnless(ServiceBinaryCatalog(paths: AppSupportPaths()).isInstalled(.mysql),
    ...
    private func loopbackProfile(readOnly: Bool) -> ConnectionProfile {
        ConnectionProfile(name: "ro-test", kind: .mysql, host: "127.0.0.1", port: 3306,
                          user: "root", database: "mysql", tlsMode: .prefer, readOnly: readOnly)
    }
    ...
    func testReadOnlySessionRejectsInsertAtServer() async throws {
        ...
        do {
            ...
            try await readonly.insert(database: scratchDB, table: "t",
