...
final class MongoDriverIntegrationTests: XCTestCase {
    ...
    private func makeDriver() throws -> MongoDriver {
        try XCTSkipUnless(ProcessInfo.processInfo.environment["KDWARM_DB_IT"] == "1",
                          "Set KDWARM_DB_IT=1 with the MongoDB engine installed + running on :27017.")
    ...
    private let database = "kdwarm_it"
    ...
    func testCRUDLifecycle() async throws {
        ...
        try await driver.insert(database: database, collection: collection,
                                json: #"{"_id":{"$oid":"507f1f77bcf86cd799439011"},"name":"alice","age":30}"#)

        var found = try await driver.find(database: database, collection: collection,
                                          filterJSON: #"{"name":"alice"}"#, limit: 10, skip: 0)
        XCTAssertEqual(found.count, 1)
        let record = try XCTUnwrap(found.first)
        XCTAssertEqual(record.id, "507f1f77bcf86cd799439011")

        try await driver.update(database: database, collection: collection, record: record,
                                json: #"{"name":"alice","age":31}"#)
        found = try await driver.find(database: database, collection: collection,
                                      filterJSON: #"{"name":"alice"}"#, limit: 10, skip: 0)
        XCTAssertTrue(found.first?.json.contains("31") ?? false)

        let aggregated = try await driver.aggregate(database: database, collection: collection,
            pipelineJSON: #"[{"$match":{"name":"alice"}},{"$project":{"name":1}}]"#, limit: 10)
        XCTAssertEqual(aggregated.count, 1)

        try await driver.delete(database: database, collection: collection, record: record)
        found = try await driver.find(database: database, collection: collection,
                                      filterJSON: nil, limit: 10, skip: 0)
    ...
    func testReadOnlyConnectionRefusesWrites() async throws {
        try XCTSkipUnless(ProcessInfo.processInfo.environment["KDWARM_DB_IT"] == "1", "Integration only.")
        let readOnly = ConnectionProfile(name: "ro", kind: .mongodb, host: "127.0.0.1", port: 27017,
                                         user: "", database: "admin", tlsMode: .disable, readOnly: true)
